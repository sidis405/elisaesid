...
class AdmincoreAdmin
{
    ...
    {
        if(is_admin()) {
            global $sndvtheme, $pagenow;
            $this->theme = $sndvtheme;
    ...
    function form_colorpicker ($name, $attributes = array())
    {
        $prefix = isset($attributes['prefix']) ? $attributes['prefix'] : '';
        $suffix = isset($attributes['suffix']) ? $attributes['suffix'] : '';
        $return = $prefix . "<input name=\"$name\"  value=\"" . $this->theme->get_option($name) . "\" class=\"color {required:false}\" style=\"border:1px solid #dcdfe4; width:250px;height:30px;  \" " .  $this->apply_attributes($attributes) . ">" . $suffix;
        return $return;
    }
    ...
    function optionsPageMenu()
    {
        ?>
            ...
            <div class="admintab">
                <?php
                    $tabids = 0;
                    $sndvtheme_optionspage = $this->do_priority($this->theme->admin_options);
                                            
                    if( is_array($sndvtheme_optionspage) && count($sndvtheme_optionspage) > 0) {
                        
                        foreach($sndvtheme_optionspage as $optionspage) {
                            $tabids++;
